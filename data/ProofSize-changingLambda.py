...
for lamb in lambs[::-1]:
    ...
    #plot mean
    ax.plot(maxBlocksPerLeaf, meanArray, label='λ=' + str(lamb))
    #plot CIs
...
plt.ylim(0,13000)
array = [x*2000 for x in range(0,7)]
for elem in array:
...
fig.savefig('./PaperPlots/L='+str(L) +'-OnlyLambNewChangingProofSizeComparison' + str( date.today() ) + '.pdf', bbox_inches='tight')
